...
def send_quick_replies(psid):
    ...
    if not now_open:
        greeting += f"⏰ Our store is currently closed. You can still browse our menu, check our location, or contact us.\n"

    msg = {
        "text": greeting + "Please choose an option:",
        "quick_replies": [
            {"content_type": "text", "title": "📋 View Menu", "payload": "Q_VIEW_MENU"},
            {"content_type": "text", "title": "🗺️ Location", "payload": "Q_LOCATION"},
...
def handle_payload(psid, payload):
    ...
    if payload == "Q_ADVANCE_ORDER":
        if not now_open:
            call_send_api(psid, {"text": f"❌ Sorry, advance orders are accepted between {OPEN_TIME.strftime('%I:%M %p')} and {CLOSE_TIME.strftime('%I:%M %p')}."})
            return send_quick_replies(psid)
